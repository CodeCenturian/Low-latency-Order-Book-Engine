...
double jnum(const std::string& json, const std::string& key) {
    auto pos = json.find("\"" + key + "\"");
...
std::string book_to_json(const OrderBook& book) {
    std::ostringstream o;
    o << std::fixed << std::setprecision(2);
    o << "{\"type\":\"book\",\"bids\":[";
...
int main()
{
    ...
    while (std::getline(std::cin, line)) {
        ...
        std::string action = jstr(line, "action");
        ...
        if (action == "buy") {
            ...
            emit("{\"type\":\"cancel_ack\",\"id\":" + std::to_string(cancel_id)
                 + ",\"ok\":" + (ok ? "true" : "false") + "}");
        ...
        if (action != "cancel") {
            emit("{\"type\":\"ack\",\"id\":" + std::to_string(id) + "}");
        }
